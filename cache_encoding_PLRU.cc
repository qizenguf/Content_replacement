...
int main(int argc, char* argv[]){
   ...
   if(!fin.good()){
		...
		if(addr[0]=='n'){ // write back hit
				...
				if(m_cache->accessSingleLine(addrValue, Cache::STORE, (Byte*)data, 64, SubsecondTime::Zero(), true) == NULL){ // miss
					...
					if(m_cache->isFull(addrValue)){ // before insertion happens, find the optimal first
						optimal_t = m_cache->lowestDiffLineEncoding_PLRU(addrValue, (Byte*)data, SubsecondTime::Zero(), optimal_index, m_shiftSize, m_flipSize, range, weight);
	...
	char foo[256];
    //int period = 0;
    sprintf(foo,"encoding2_LW%d_r%d_%s_%dM_%dway_f%d.txt", weight,  range, workload.c_str(), cacheSize, associativity, m_flipSize);
