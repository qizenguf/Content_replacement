...
int main(int argc, char* argv[]){
   ...
   if(!fin.good()){
		...
		if(addr[0]=='n'){ // write back hit
				...
				if(m_cache->accessSingleLine(addrValue, Cache::STORE, (Byte*)data, 64, SubsecondTime::Zero(), true) == NULL){ // miss
					...
					if(m_cache->isFull(addrValue)){ // before insertion happens, find the optimal first
					   ...
					   optimal_t = m_cache->lowestDiffLine(addrValue, (Byte*)data, SubsecondTime::Zero(), optimal_index, m_shiftSize, m_flipSize, range);
	...
	char foo[256];
    //int period = 0;
    sprintf(foo,"oracle_sec%d_%s_sec%d_%dM_%dway_L%dm.txt", range, workload.c_str(), sector_size, cacheSize, associativity, length);
