...
int main(int argc, char* argv[]){
	...
	int m_flipSize = 2;
	if(argc > 6) m_flipSize = atoi(argv[6]);
	int samples = 64;
	if(argc > 7) samples = atoi(argv[7]);
	int sample_interval = num_sets/samples;
	int duel_length = 100000;
	if(argc > 8) duel_length= atoi(argv[8]);
    ...
    Cache * m_cache = new Cache("L3",
     ...
     // m_shmem_perf_global = new ShmemPerf();
      ...
      int m_shiftSize = 64, m_sample_size = 2 , m_sample_interval = 32, m_sample_start = 0;
   ...
   if(!fin.good()){
	...
	uint64_t start = 0;
	uint64_t nlru_counter = 0;
	uint64_t plru_counter = 0;
	int current_policy = 0; // 0 for NLRU, 1 for PLRU
	int plru_policy_change = 0 ;
	int nlru_policy_change = 0;
	int plru_period_cnt = 0;
	int nlru_period_cnt = 0;
	int nlru_access = 0;
	int plru_access = 0;
	int plru_miss = 0;
	int nlru_miss = 0;
    ...
    while(!fin.eof()){
		...
		if(addr[0]=='n'){ // write back hit
				...
				UInt32 set_index = m_cache->getSetIndex(addrValue);
				int set_type = 0;
				int tt= set_index % sample_interval; // sample method
				int res = set_index/sample_interval;
				double ratio = samples/sample_interval;
				int id = res/ratio;
				if(id == tt) {set_type = 1; nlru_access++;}
				else if(id == sample_interval - tt ) {set_type = 2; plru_access++; }
				...
				if(m_cache->accessSingleLine(addrValue, Cache::STORE, (Byte*)data, 64, SubsecondTime::Zero(), true) == NULL){ // miss
					...
					if(m_cache->isFull(addrValue)){ // before insertion happens, find the optimal first
						...
						if(set_type == 1 ) // group 1 nlru
							optimal_t = m_cache->lowestDiffLineEncoding_NLRU(addrValue, (Byte*)data, SubsecondTime::Zero(), optimal_index, m_shiftSize, m_flipSize, range, weight);
						else if(set_type == 2 ) // group 2 plru							
							optimal_t = m_cache->lowestDiffLineEncoding_NLRU(addrValue, (Byte*)data, SubsecondTime::Zero(), optimal_index, m_shiftSize, m_flipSize, range, 513);
						else if(current_policy == 0) //policy nlru
							optimal_t = m_cache->lowestDiffLineEncoding_NLRU(addrValue, (Byte*)data, SubsecondTime::Zero(), optimal_index, m_shiftSize, m_flipSize, range, weight);
						else if(current_policy == 1) //policy plru
							optimal_t = m_cache->lowestDiffLineEncoding_NLRU(addrValue, (Byte*)data, SubsecondTime::Zero(), optimal_index, m_shiftSize, m_flipSize, range, 513);
						//std::cout<<addrValue <<" with sample2 " << sample_t<<std::endl;
					  ...
					  // sample_lru = m_cache->getReplacementInfo(addrValue, sample_index);
						...
						//std::cout<<addrValue<<" with sample44 " << optimal_index<<std::endl;
					}
					//actual insertion here!!!!!!
					eviction = false;
					//std::cout<<addrValue<<" with sample4 " << optimal_index<<std::endl;
					m_cache->insertSingleLinewIndex(addrValue, (Byte*)data, &eviction, &evict_addr, &evict_block_info, evict_buf, SubsecondTime::Zero(), optimal_index);	
					//std::cout<<addrValue<<" with sample44 " << optimal_index<<std::endl;
					...
					// assert(set_index == set_index2);        
					 if(eviction) {
						 ...
						 code_d = m_cache->encodingCompare ((Byte*)data, evict_buf, 64, m_shiftSize, m_flipSize);
					     ...
					     code_diff[code_d] ++;
					     if(addrValue/4096 == evict_addr/4096) {
							evicted_nearby++;
						...
						if(set_type == 1) {nlru_counter += optimal_t; nlru_miss++;}
						else if (set_type == 2 ) {plru_counter += optimal_t; plru_miss++;}
		...
		if(count % duel_length== 0) {
			
			if(current_policy == 0) { // nlru
				nlru_period_cnt++;
				int diff = nlru_counter - plru_counter;
				if (nlru_counter != 0 && ( plru_counter == 0 || diff > nlru_counter * 0.10 ) ){
					current_policy = 1;
					std::cout<<workload.c_str()<<" at "<<count/duel_length<<" switched to plru "<< diff/(double)nlru_counter <<" diff "<<diff <<endl;
					std::cout<<workload.c_str()<<" plru "<<plru_miss<< " / " <<plru_access<< " mr "<<(double)plru_miss/plru_access<<" with "<< plru_counter << " flips ; nlru "<<nlru_miss <<" / "<<nlru_access<<  " mr "<<(double)nlru_miss/nlru_access<<" with "<< nlru_counter <<" flips"<<endl;
					plru_policy_change++;
				}
					
				
			}
			else if(current_policy == 1 ){ //plru
				plru_period_cnt++;
				int diff =  plru_counter - nlru_counter;
				if (plru_counter!= 0 && (nlru_counter == 0 || diff > nlru_counter * 0.10) ){
					current_policy = 0 ;
					std::cout<<workload.c_str()<<" at "<<count/duel_length<<" switched to nlru "<< diff/(double)nlru_counter<<" diff "<<diff<<endl;
					std::cout<<workload.c_str()<<" plru "<<plru_miss<< " / " <<plru_access<< " mr "<<(double)plru_miss/plru_access <<" with "<< plru_counter << " flips ; nlru "<<nlru_miss <<" / "<<nlru_access<< " mr "<<(double)nlru_miss/nlru_access <<" with "<< nlru_counter <<" flips"<<endl;
					nlru_policy_change++;
				}
			}
	...
	char foo[256];
    //int period = 0;
    sprintf(foo,"duelnew_LW%d_r%d_%s_%dM_%dway_f%ds%dd%d.txt", weight,  range, workload.c_str(), cacheSize, associativity, m_flipSize, samples, duel_length);
	...
	fout<<" change to plru " <<plru_policy_change<<"  ; change to nlru "<< nlru_policy_change << " total= " << nlru_policy_change + plru_policy_change<<endl;
	fout<<"nlru " <<nlru_period_cnt<<" plru "<<plru_period_cnt << " n/(n+p) = " << (double)nlru_period_cnt/(nlru_period_cnt+plru_period_cnt) <<endl;
