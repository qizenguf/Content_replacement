...
for a in $LIST
	do
	printf "%s \t \t" $a
	cat encodingNLRU_LW0_r8_trace-$a'_2M_32way_f8.txt' | grep "evicted_bits" | awk '{printf "%d\t %d\t", $2, $3}'
	cat encodingNLRU_LW0_r8_trace-$a'_2M_32way_f8.txt' | grep "hit rate" | awk '{printf "%f\t", $4}'
