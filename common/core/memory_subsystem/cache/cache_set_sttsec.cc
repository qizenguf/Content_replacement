...
CacheSetSTTSEC::CacheSetSTTSEC(
   ...
   , m_set_info(set_info)   
{  
   m_mru_cutoff = associativity/4; // front mru blocks are protected fron eviction
...
CacheSetSTTSEC::getReplacementIndex(CacheCntlr *cntlr)
{
   ...
   // Make m_num_attemps attempts at evicting the block at LRU position
    ...
    //  LOG_ASSERT_ERROR(index < m_associativity, "Error Finding LRU bits");
      ...
      /*else if( num_of_nearby > m_nearby_thres){ // if the sector is almost full, we will ignore the mru cut_off
		  moveToMRU(index);
		  return index;
	  }*/
	  else{
...
UInt8 CacheSetSTTSEC::getReplacementInfo(UInt32 accessed_index)
{
